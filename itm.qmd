...
E(X) = \int_{0}^{k} x \cdot x^3 dx = \int_{0}^{k} x^4 dx\\
...
Step 1
$$
E(X^2) = \int_{0}^{k} x^2 \cdot x^3 dx = \int_{0}^{k} x^5 dx\\
...
and (b) write an R
function to generate random numbers from X ∼ fX(x) that relies on the ITM.
```{r}
...
8. Compare the range of the numbers generated at point (7) to the support of X. What
do you notice?
...
9. Compute the mean and variance of the numbers generated at point (7), and compare
them to the values of E(X) and V ar(X) that you computed at point (3). Are the
numerical results in agreement with your analytical derivations?
...
10. Draw a histogram that shows the empirical density of the 10000 random numbers, and
compare it to fX(x)
```{r}
